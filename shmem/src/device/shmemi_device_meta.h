...
constexpr uint64_t SMEM_SHM_DEVICE_META_UDMA_INFO_OFFSET = SMEM_SHM_DEVICE_META_QP_INFO_OFFSET + sizeof(uint64_t);

ACLSHMEM_DEVICE void aclshmemi_check_shmem_id_valid(uint32_t shmemId)
{
    if (shmemId >= SMEM_OBJECT_NUM_MAX) {
        aclshmemi_kernel_abort("invalid shmemId=%u, max=%lu\n", shmemId, SMEM_OBJECT_NUM_MAX);
    }
}
...
ACLSHMEM_DEVICE __gm__ void* aclshmemi_get_udma_info_address(uint32_t shmemId)
{
    ACLSHMEM_DEBUG_FUNC(aclshmemi_check_shmem_id_valid, shmemId);

    uint64_t metaAddr =
        SMEM_SHM_DEVICE_META_ADDR + SMEM_SHM_DEVICE_GLOBAL_META_SIZE + shmemId * SMEM_SHM_DEVICE_PRE_META_SIZE;
    return *(__gm__ void**)(metaAddr + SMEM_SHM_DEVICE_META_UDMA_INFO_OFFSET);
}
