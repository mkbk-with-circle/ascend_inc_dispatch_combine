...
run_one_case() {
    ...
    local csv_path="${OUTPUT_DIR}/dispatch_perf_rank${prof_pe_value}.csv"
    ...
    echo "[Dispatch] Running ${case_id}, prof_pe=${prof_pe_value}, ipport=${run_ipport}, session=${SHMEM_UID_SESSION_ID}"
    ...
    for ((idx = 0; idx < pes; idx++)); do
        ${PROJECT_ROOT}/build/bin/dispatch "${pes}" "${idx}" "${run_ipport}" "${pes}" "${FIRST_PE}" "${FIRST_NPU}" \
...
if [[ "${PERF_MODE}" == "1" ]]; then
    ...
    if [[ "${GNPU_NUM}" -ne "${PE_SIZE}" ]]; then
        echo "Error: dispatch run.sh expects -gnpus to equal -pes for this single-node example."
        exit 1
    fi
...
if [[ "${PERF_MODE}" == "1" && "${PROF_PE}" == "all" ]]; then
    python3 "${PROJECT_ROOT}/examples/utils/summarize_moe_perf.py" --dir "${OUTPUT_DIR}" \
        --output "dispatch_perf_summary.csv"
fi
