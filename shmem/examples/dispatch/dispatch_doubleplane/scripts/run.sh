...
TEST_TYPE="int32_t"
BS="8"
# Fixed hidden size (7k); do not sweep via --h-list unless explicitly needed.
H="7168"
TOPK="8"
EXPERT_PER_PE="2"
PERF_MODE="0"
...
run_one_case() {
    ...
    if (( expert_per_pe <= 0 || expert_per_pe > MAX_LOCAL_EXPERT_NUM )); then
        echo "Error: expertPerPe must be in [1, ${MAX_LOCAL_EXPERT_NUM}], got ${expert_per_pe}."
        return 1
    fi
    ...
    local csv_path="${OUTPUT_DIR}/dispatch_doubleplane_perf_rank${prof_pe_value}.csv"
    local case_port=$((8766 + CASE_INDEX % 1000))
    local session_port=$((8899 + CASE_INDEX % 1000))
    ...
    echo "[DispatchDoubleplane] Running ${case_id}, prof_pe=${prof_pe_value}, ipport=${run_ipport}, session=${SHMEM_UID_SESSION_ID}"
    mkdir -p output "${OUTPUT_DIR}"
    rm -f output/expand_x_*.bin output/assist_info_*.bin output/ep_recv_count_*.bin output/expert_token_nums_*.bin
    ...
    for ((idx = 0; idx < pes; idx++)); do
        ${PROJECT_ROOT}/build/bin/dispatch_doubleplane "${pes}" "${idx}" "${run_ipport}" "${pes}" "${FIRST_PE}" "${FIRST_NPU}" \
    ...
    python3 "${GROUP_DIR}/scripts/check_dispatch.py" --pes "${pes}" --bs "${bs}" --h "${h}" --topk "${topk}" \
        --expert-per-pe "${expert_per_pe}" --dtype "${TEST_TYPE}"
}
...
if [[ "${PERF_MODE}" == "1" ]]; then
    ...
    if [[ "${GNPU_NUM}" -ne "${PE_SIZE}" ]]; then
        echo "Error: dispatch_doubleplane run.sh expects -gnpus to equal -pes for this single-node example."
        exit 1
    fi
...
if [[ "${PERF_MODE}" == "1" && "${PROF_PE}" == "all" ]]; then
    python3 "${PROJECT_ROOT}/examples/utils/summarize_moe_perf.py" --dir "${OUTPUT_DIR}" \
        --output "dispatch_doubleplane_perf_summary.csv"
fi
