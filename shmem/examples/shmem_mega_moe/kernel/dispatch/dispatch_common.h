...
/*!
 * \file dispatch_common.h
 * \brief
 */

#ifndef SHMEM_EXAMPLES_MEGA_MOE_KERNEL_DISPATCH_COMMON_H
#define SHMEM_EXAMPLES_MEGA_MOE_KERNEL_DISPATCH_COMMON_H
...
static __aicore__ inline void HistogramsVf(
    __local_mem__ U* dst, __local_mem__ T* src, uint16_t repeatElm, uint16_t halfRepeat, uint32_t totalElm,
...
__aicore__ inline void GetExpertFreq(LocalTensor<uint16_t>& dstLocal, LocalTensor<uint8_t>& srcLocal, uint32_t totalElm)
{
    ...
    VF_CALL<HistogramsVf<MicroAPI::HistogramsType::FREQUENCY, uint8_t, uint16_t>>(
        dst, src, repeatElm, repeatElm >> 1, totalElm, repeatTimes);
    PipeBarrier<PIPE_V>();
}

__aicore__ inline void GetExpertCumSum(
    LocalTensor<uint16_t>& dstLocal, LocalTensor<uint8_t>& srcLocal, uint32_t totalElm)
{
    uint32_t repeatElm = GetVecLen();
    uint16_t repeatTimes = Ceil<uint32_t, uint16_t>(totalElm, repeatElm);
    __local_mem__ uint8_t* src = (__local_mem__ uint8_t*)srcLocal.GetPhyAddr();
    __local_mem__ uint16_t* dst = (__local_mem__ uint16_t*)dstLocal.GetPhyAddr();
    VF_CALL<HistogramsVf<MicroAPI::HistogramsType::ACCUMULATE, uint8_t, uint16_t>>(
        dst, src, repeatElm, repeatElm >> 1, totalElm, repeatTimes);
    PipeBarrier<PIPE_V>();
}

static __aicore__ inline void ReduceLoop(
    __local_mem__ int32_t* dst, __local_mem__ int32_t* src, uint16_t repeat0Times, uint32_t repeat0SrcStride,
    uint16_t repeat1Times, uint32_t repeat1Stride, uint32_t repeat1Element, uint32_t repeat0DstStride)
{
    ...
    for (uint16_t i0 = 0; i0 < repeat0Times; ++i0) {
        ...
        for (uint16_t i1 = 0; i1 < repeat1Times; ++i1) {
            ...
            uint32_t remaining = repeat1Element - static_cast<uint32_t>(i1) * repeat1Stride;
            uint32_t elements = remaining < repeat1Stride ? remaining : repeat1Stride;
        ...
        MicroAPI::DataCopy<int32_t, MicroAPI::StoreDist::DIST_FIRST_ELEMENT_B32>(
            dst + i0 * repeat0DstStride, sumReg, maskFirst);
    }
}

__aicore__ inline void GetReduceSum(
    LocalTensor<int32_t>& dstLocal, LocalTensor<int32_t>& srcLocal, uint16_t repeat0Times, uint32_t repeat0SrcStride,
    uint32_t repeat1Element, uint32_t repeat0DstStride)
{
    ...
    VF_CALL<ReduceLoop>(
        dst, src, repeat0Times, repeat0SrcStride, repeat1Times, repeat1Stride, repeat1Element, repeat0DstStride);
    PipeBarrier<PIPE_V>();
}

} // namespace AscendC

#endif // SHMEM_EXAMPLES_MEGA_MOE_KERNEL_DISPATCH_COMMON_H
