...
把 `common/api` 的 Framework backend vtable **接到真实 ACLSHMEM kernel**。  
这里的 `native` 表示「原生/真实 backend」，**不是** naive 实现。

### 为什么要有 runtime 层？

- 公开 C API 不能直接 `#include` 设备 kernel；需要 session、workspace、mailbox、generation rendezvous。
- Dispatch 与 Combine 各有 backend，再用 **composite 合成一个 communicator** 后注入 Framework/Easy（调用链是 Framework→composite→各 backend，不是 Framework 先碰两个裸 backend 再合成）。
...
`BindNativeSingleIncWorkerControl(&control, &single_inc_config)`；之后只走
`inc_dc_single_inc_dispatch/combine`。

`inc_dc_native_full_example_main.cpp` 仍显式展开底层 Easy/service 步骤，因为它还要
...
| `inc_dc_native_composite_backend.h` / `.cpp` | 把 D/C 两个 backend 合成一个 framework communicator | Easy/Inference 只需持有一个 comm | 必需 |
...
Adapters that bind the Framework backend vtable in `common/api` to **real
ACLSHMEM kernels**. Here `native` means a real provider—not a naive stub.

### Why a runtime layer?

- The public C API must not include device kernels directly; it needs sessions,
  workspaces, mailboxes, and generation rendezvous.
- Separate Dispatch/Combine backends plus a composite communicator keep Easy /
  Inference injection simple.
...
| `inc_dc_native_composite_backend.*` | One communicator over both backends | Single handle for Easy/Inference | Required |
