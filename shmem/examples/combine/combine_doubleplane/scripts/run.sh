...
run_one_case() {
    ...
    local csv_path="${OUTPUT_DIR}/combine_doubleplane_perf_rank${prof_pe_value}.csv"
    ...
    for ((idx = 0; idx < pes; idx++)); do
        ${PROJECT_ROOT}/build/bin/combine_doubleplane "${pes}" "${idx}" "${run_ipport}" "${pes}" "${FIRST_PE}" "${FIRST_NPU}" \
...
if [[ "${PERF_MODE}" == "1" ]]; then
    ...
    if [[ "${GNPU_NUM}" -ne "${PE_SIZE}" ]]; then
        echo "Error: combine_doubleplane run.sh expects -gnpus to equal -pes for this single-node example."
        exit 1
    fi
...
if [[ "${PERF_MODE}" == "1" && "${PROF_PE}" == "all" ]]; then
    python3 "${PROJECT_ROOT}/examples/utils/summarize_moe_perf.py" --dir "${OUTPUT_DIR}" \
        --output "combine_doubleplane_perf_summary.csv"
fi
