/*
 * 单 INC Inference API：可在已配置 CANN toolchain 的 host 上运行的完整示例。
 *
...
int main()
{
    ...
    inc_dc_infer_request_t dispatch_request{};
    inc_dc_infer_request_t combine_request{};
    inc_dc_infer_route_t captured_route{};
    bool dispatch_live = false;
    bool combine_live = false;
    bool route_live = false;
    int exit_code = EXIT_FAILURE;
    inc_dc_fw_status_t status = INC_DC_FW_OK;
    uint32_t request_state = INC_DC_FW_REQUEST_FREE;
    uint64_t route_bytes = 0u;

    inc_dc_infer_config_t config{};
    inc_dc_infer_plan_desc_t plan_desc{};
    inc_dc_infer_plan_info_t plan_info{};
    inc_dc_easy_token_plan_desc_t route_plan_desc{};
    inc_dc_easy_token_plan_info_t route_info{};
    inc_dc_infer_io_t dispatch_io{};
    inc_dc_infer_io_t combine_io{};
...
#define CHECK_OK(expression)                                                   \
    ...
    std::printf("=== 单 INC Inference API / CPU mock 完整示例 ===\n");

    /* 1) 配置 communicator、注入 backend/allocator，然后初始化 session。 */
    inc_dc_infer_config_init(&config);
    config.communicator.session_id = 1u;
    config.communicator.model_id = 7u;
    config.communicator.process_group_id = 11u;
    config.communicator.topology_generation = 1u;
    config.communicator.worker_world_size = kWorldSize;
    config.communicator.worker_rank = 0u;
    config.communicator.max_tokens_per_chunk = 128u;
    config.communicator.hidden_size = kHidden;
    config.communicator.max_topk = kTopK;
    config.communicator.dtype = INC_DC_FW_DTYPE_FP16;
    config.communicator.max_inflight = 8u;
    config.communicator.max_workspace_queries = 8u;
    config.communicator.backend = MakeMockBackend(&backend);
    config.allocate_device = MockAllocate;
    config.free_device = MockFree;
    config.allocator_context = &backend;
    CHECK_OK(inc_dc_infer_session_create(&config, &session));
    std::printf("[1/8] session 初始化完成\n");

    /* 2) prepare once：按固定 (tokens, topk) bucket 建立持久 plan/workspace。 */
    inc_dc_infer_plan_desc_init(&plan_desc);
    plan_desc.tokens = kTokens;
    plan_desc.topk = kTopK;
    CHECK_OK(inc_dc_infer_plan_create(session, &plan_desc, &plan));
    CHECK_OK(inc_dc_infer_plan_get_info(plan, &plan_info));
    std::printf(
     ...
     */
    inc_dc_infer_io_init(&dispatch_io);
    ...
    /* 4) 异步 Dispatch，并在 request 释放前捕获本次精确 route。 */
    CHECK_OK(inc_dc_infer_dispatch_async(
        plan, &dispatch_io, &dispatch_request));
    dispatch_live = true;
    CHECK_OK(inc_dc_infer_route_capture(
        plan, &dispatch_request, &dispatch_io, &captured_route));
    route_live = true;
    status = inc_dc_infer_request_query(
        plan, &dispatch_request, &request_state);
    if (status != INC_DC_FW_NOT_READY ||
        request_state != INC_DC_FW_REQUEST_SUBMITTED) {
        std::fprintf(stderr, "Dispatch query 未返回预期的 SUBMITTED 状态\n");
        goto cleanup;
    }
    CHECK_OK(inc_dc_infer_request_wait_and_release(
        plan, &dispatch_request, 0u));
    dispatch_live = false;
    std::printf("[3/8] Dispatch 完成并释放 request；route handle 仍然有效\n");
    ...
    inc_dc_infer_io_init(&combine_io);
    ...
    CHECK_OK(inc_dc_infer_combine_with_route_async(
        plan, &captured_route, &combine_io, &combine_request));
    combine_live = true;
    CHECK_OK(inc_dc_infer_request_wait_and_release(
        plan, &combine_request, 0u));
    combine_live = false;
    std::printf("[5/8] Combine 完成并释放 request\n");
    ...
    CHECK_OK(inc_dc_infer_session_get_stats(session, &stats));
    std::printf(
...
cleanup:
    /*
     * 8) 严格按 request -> route -> plan -> session 的依赖顺序释放。
     * 真实调度器同样应在异常路径执行这些动作；live request 可先 cancel。
     */
    if (combine_live) {
        (void)inc_dc_infer_request_cancel(plan, &combine_request);
        (void)inc_dc_infer_request_release(plan, &combine_request);
    }
    if (dispatch_live) {
        (void)inc_dc_infer_request_cancel(plan, &dispatch_request);
        (void)inc_dc_infer_request_release(plan, &dispatch_request);
    }
    if (route_live) {
        const inc_dc_fw_status_t cleanup_status =
            inc_dc_infer_route_release(session, &captured_route);
        if (cleanup_status != INC_DC_FW_OK) exit_code = EXIT_FAILURE;
    }
    if (plan != nullptr) {
        const inc_dc_fw_status_t cleanup_status =
            inc_dc_infer_plan_destroy(plan);
        if (cleanup_status != INC_DC_FW_OK) exit_code = EXIT_FAILURE;
        plan = nullptr;
    }
    if (session != nullptr) {
        const inc_dc_fw_status_t cleanup_status =
            inc_dc_infer_session_destroy(session);
        if (cleanup_status != INC_DC_FW_OK) exit_code = EXIT_FAILURE;
        session = nullptr;
    }
