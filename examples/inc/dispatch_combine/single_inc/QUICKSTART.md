...
```text
Inference API  →  Easy API  →  Framework C API (backend vtable)
                                      ↓
...
CompileLogicalPlanToExecution      → IncDcCompiledExecutionPlan
        │                              （home_inc/owner、ingress channel/slot、worklist CSR…）
        ▼
...
3. Owner 只处理 `home_inc/home_owner` 匹配的 result。  
